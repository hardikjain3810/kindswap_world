 ...
 * Must be used in conjunction with AdminGuard.
 *
 * Usage:
...
export class SuperAdminGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Extract admin wallet (should be set by AdminGuard)
    ...
    // First, check if AdminGuard already set the isSuperAdmin flag
    // This avoids redundant database queries (N+1 optimization)
    let isSuperAdmin = (request as any).isSuperAdmin;

    // Only query database if flag wasn't set by AdminGuard
