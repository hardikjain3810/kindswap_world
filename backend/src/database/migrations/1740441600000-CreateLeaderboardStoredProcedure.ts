...
export class CreateLeaderboardStoredProcedure1740441600000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const hasSwapTable = existingTables.has('swap_transactions');
    ...
    // CRITICAL: Only create stored procedure if at least swap_transactions exists
    if (!hasSwapTable) {
      console.log('⚠️  swap_transactions table does not exist - skipping stored procedure creation');
      return;
    }
    ...
    `);
      ...
      // If stored procedure creation fails, log but continue
