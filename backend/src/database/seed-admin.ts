...
async function seedSuperAdmin() {
  ...
  try {
    ...
    if (existingSuperAdmin) {
      ...
      console.log(`[SEED]   Permissions: ${existingSuperAdmin.permissions.join(', ') || 'None'}`);
    ...
    const superAdmin = adminRepository.create({
      ...
      isSuperAdmin: true,
      permissions: [AdminPermission.FEE_CONFIG, AdminPermission.CONTRIBUTIONS], // Super Admin gets all permissions
      isActive: true,
    });
    ...
    console.log(`[SEED]   Permissions: ${superAdmin.permissions.join(', ')}`);
